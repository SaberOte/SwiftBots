...
class ChatMessageHandler(IChatMessageHandler):
    ...
    def __init__(self, controllers: List[IController], logger: ILogger):
        ...
        commands_represent = "\n".join(
            [
                f'"{command.command_name}": ({command.controller.__class__.__name__}){command.method}'
    ...
    def use_as_default(self, controller: Optional[IController]) -> None:
        ...
        if controller is not None:
            self.__logger.debug(
                f"Registered default handler directly: {self.__default_controller}"
            )
        else:
    ...
    async def handle_message_async(self, view: IView, context: "IContext") -> None:
        ...
        assert isinstance(
            view, IChatView
        ), "Chat message handler can only take IChatView views"
        ...
        if best_matched_command:
            ...
            self.__default_controller is not None
        ):  # No matches. Use default controller method
            command_name = arguments = message
            controller = self.__default_controller
            method = controller.default.__func__  # type:ignore
        ...
        del context["message"]
        new_context = view.associated_context(
            raw_message=message, arguments=arguments, command=command_name, **context
        )
    ...
    def __register_default_controller(self) -> None:
        ...
        if len(defaults) == 0:
            ...
            self.__logger.debug(
                f"One default handler found and registered: {self.__default_controller}"
            )
        else:
            self.__logger.warning(
                f"Multiple default handlers found in controllers: {defaults}"
            )
            self.__default_controller = defaults[0]
